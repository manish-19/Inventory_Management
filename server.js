...
//homepage
app.get('/', (req,res)=>{                
	db.collection('Bike').find().toArray( (err,result)=>{     
		if(err) return console.log(err)     
...
app.post('/AddData', (req,res)=>{			
	db.collection('Bike').save(req.body, (err,result)=>{			
		if(err) return console.log(err)
	res.redirect('/')                        
	})
})
...
app.post('/update', (req,res)=>{			
		db.collection('Bike').find().toArray((err,result)=>{	   			
			...
			db.collection('Bike').findOneAndUpdate({pid:req.body.id},{
...
app.post('/delete',(req,res)=>{
	db.collection('Bike').findOneAndDelete({pid:req.body.id}, (err,result)=>{    
		if(err)
			return console.log(err)
		res.redirect('/')
	})
